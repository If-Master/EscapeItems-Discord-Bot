...
def format_shop_item_row(entry: dict) -> str:
    mode = "Selling" if entry["is_selling"] else "Buying"
    qty_label = f"{entry['quantity']} SB" if entry["is_shulker"] else str(entry["quantity"])
    draft_tag = " [DRAFT]" if entry.get("is_draft") else ""
    return (
        f"**{entry['item_name']}**{draft_tag} — "
        f"{mode} | Qty: {qty_label} | Price: {entry['price']:.2f}"
    )
